...
class UserCallsController extends Controller
{
    ...
    public function show(string $callId): JsonResponse
    {
        ...
        $call = $this->resolveCall($callId, $user->company_id, [
        ...
        if ($call->category_id) {
            $jobHistory->push([
                ...
                'detail'     => $call->category?->name ? 'Category: '.$call->category->name : 'Category assigned',
            ]);
    ...
    public function regenerate(string $callId): JsonResponse
    {
        $user = Auth::guard('web')->user();
        ...
        $call = $this->resolveCall($callId, $user->company_id, ['company:id,timezone']);
        ...
        if (! $aiRecovery['hasTranscript']) {
            return response()->json([
                ...
                'data'    => ['call_id' => $call->id, 'action' => 'none'],
            ], 422);
        }
    ...
    public function checkTranscript(string $callId): JsonResponse
    {
        $user = Auth::guard('web')->user();
        ...
        try {
            ...
            Log::warning('UserCallsController: checkTranscript fetch failed', [
                'call_id'      => $call->id,
    ...
    private function resolveCall(string $idOrUid, int $companyId, array $with = []): ?Call
    {
        $query = Call::query()->with($with)->where('company_id', $companyId);
