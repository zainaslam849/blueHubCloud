...
// The dropdown's usual options come from a separate, filtered/paginated
// user list (role=user, first 200) — the user actually assigned to the
// company being edited isn't guaranteed to be in it (more than 200
// registered users, or a role stored differently). Keep them around
// separately so they can always be shown as a selectable option even when
// the general list doesn't happen to include them.
const editingAssignedUser = ref(null); // { id, name, email } | null

const assignUserOptions = computed(() => {
    if (
        !editingAssignedUser.value ||
        registeredUsers.value.some((u) => u.id === editingAssignedUser.value.id)
    ) {
        return registeredUsers.value;
    }
    return [editingAssignedUser.value, ...registeredUsers.value];
});
...
function openAddForm() {
    ...
    currentAvailableTenants.value = [];
    editingAssignedUser.value = null;
    loadRegisteredUsers();
    showForm.value = true;
}
...
function openEditForm(company) {
    ...
    // Guarantee the currently-assigned user is selectable in the dropdown
    // even if they don't happen to be in the general (filtered/paginated)
    // registeredUsers list — see assignUserOptions.
    editingAssignedUser.value = company.assigned_user_id
        ? { id: company.assigned_user_id, name: company.assigned_user_name, email: company.assigned_user_email }
        : null;
    validationErrors.value = {};
